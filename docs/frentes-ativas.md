...
| `e2-abstracao-storage` | **transversal** — Shared + 9 domínios | **não** | `app/src/Shared/`, `app/config/services.yaml`, `app/src/Shared/CLAUDE.md`, + 35 arquivos de produção | **E2.4A entregue**: os 14 uploads HTTP gravam por chave (`FonteDeUploadHttp` + `ChavesDe*::novo*`), publicação em dois passos no backend local. E2.0 `d15535fd`, congelamento `80ac07cb`, E2.1 `f37d5706`, E2.2 `fbf5b778`, E2.3 `84bded8d`. D1–D11 ratificadas. Próxima: E2.4B (4 escritas internas) | `origin/master` @ `c365fe72` |
