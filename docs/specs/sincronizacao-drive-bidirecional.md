...
- O motor de código (pastas + arquivos) está **completo e revisado**. O que falta na Fase 1 é a **execução** da carga manual supervisionada (§10.6), que depende dos **bloqueios de ops** abaixo (service account real, Shared Drive, backup+disco) — nada de código.
- Ao ligar o `GoogleDriveClient` real: fechar o **Fork 2 (D8)** — trocar o `enviarArquivo`/`baixarArquivo` in-memory por **upload resumável + download streaming** (procurar o `TODO (Fork 2 / D8)` em `ReconciliarCommand` e nos métodos do client). Validar manualmente contra um Shared Drive de teste (fora do CI). Isso elimina o risco R1 de OOM em arquivo grande.
