...
| Fase 1b — Automático (cron) | ✅ | 2026-07-13 — app OAuth publicado (token permanente), creds em `/opt/jusprime/.sync-oauth.env`, wrapper `sync-reconciliar.sh`, cron `*/15` (tenant 1) — **baixado para `0 * * * *` na ativação da Fase 2 (2026-07-15)** e, desde 2026-08, rodando `--modo=enviar` (R2). Backup do banco religado (`backup-db.sh`, cron 02:30) |
...
**✅ INTEGRAÇÃO + DEPLOY FEITOS (2026-07-10).** O sync foi integrado ao master (merge `0e64dce`, 8 conflitos resolvidos, suíte verde, feature-review sem bloqueante, smoke DEV aprovado) e **deployado em prod**; depois entrou a flag `--skip-arquivos` (`8936379`, atual `master`/`origin/master`). Migration `144054` aplicada e verificada em prod (colunas drive existem).

**🛑 FASE 1a EM EXECUÇÃO — PARAMOS ANTES DO RECONCILE REAL (2026-07-10). RETOMAR = 1 comando (P.O. já aprovou).**
- **Backup completo FEITO** (banco + 11GB de uploads via `docker run … tar` no volume `jusprime_uploads_prod`). **OAuth de ESCRITA FEITO** (`rclone authorize "drive" <id> <secret>`, escopo `https://www.googleapis.com/auth/drive` verificado) — mas o token ficou em **export no shell da VPS (efêmero)**; ao retomar, **regenerar** (rclone authorize) e re-exportar as 5 vars, com **`GOOGLE_DRIVE_CREDENTIALS='oauth-mode-unused'`** (dummy não-vazio: construtor exige `string` e o `default:` do Symfony trata vazio como null → crash; OAuth ignora o valor). client_id conhecido; client_secret (nova gerada hoje) no Google Cloud `bluejus-sync`.
- **Mapeamento REGENERADO** (o antigo `tmp/acervo-fase2/output/mapeamento.csv` de mai é PRÉ-saneamento — não usar). Insumos em `tmp/sync-fase1a/` (`pastas_prod_20260710.csv`, `drive_atual.json`, `casar.py`, `classificar.py`, `mapeamento_novo.csv`=986 vínculos).
- **BACKFILL FEITO em prod (986/0/0):** as 986 existentes já têm `drive_folder_id`. Rollback: `UPDATE pasta SET drive_folder_id=NULL WHERE tenant_id=1`.
- **DRY-RUN só-pastas CONFERIDO:** `reconciliar --tenant-id=1 --usuario-id=1 --skip-arquivos --dry-run` → **48 criadas no sistema** (40 A/B + 8 reais: 029/105/1190/505/551/606/694/696) + **1 no Drive** (pasta 1188 MARCIA, só-sistema) + **17 D9-puladas** (13 vazias + 307 `(PASTA VAZIA)` + 2 REMOVIDO + PASTA-EQUIPE — o parser pula "VAZIA" sozinho, então as vazias NÃO entram e é o correto) + 426 divergências de nome (só reporta).
- **➡️ PRÓXIMO: rodar o REAL** (mesmo comando SEM `--dry-run`, via `docker exec -e GOOGLE_DRIVE_* … app:sync:reconciliar --tenant-id=1 --usuario-id=1 --skip-arquivos`), conferir as 48 no Expediente + a MARCIA no Drive.

**Depois das 48 pastas: sincronização de ARQUIVOS (fase própria) + Fase 1a-arquivos**
- **Auth (escrita) JÁ FEITO**, mas o token é **efêmero** (só no shell da VPS, não no `.env.prod`) → regenerar ao retomar. Modo Teste → expira ~7 dias; para o cron, publicar o app e mover as vars p/ `.env.prod`.
- **A carga de ARQUIVOS é o beast:** o dry-run mostrou ~66 sobem + ~65 baixam **só para 3 pastas** → nas 1048, é os ~11GB / dezenas de milhares de arquivos, bidirecional. **NÃO rodar sem `--skip-arquivos`** até fechar o **Fork 2 (D8)** — trocar `enviarArquivo`/`baixarArquivo` in-memory por **upload resumável + download streaming** (`TODO (Fork 2 / D8)` no client). Elimina o R1 (OOM). O backup de **uploads** segue DESLIGADO no `scripts/backup.sh` (foi feito um one-off manual) — religar de vez antes do pull-down de arquivos.
