...
**RegistrarPagamento** — Quem: gestor autorizado. O quê: registrar um pagamento monetário (total ou parcial) confirmado manualmente (SPEC §11), **alocando explicitamente** quanto abateu cada obrigação do caso. Pré: caso existe no MESMO tenant e NÃO está encerrado; toda obrigação alocada pertence ao MESMO caso (invariável 12 — pagamento não atravessa casos). Regra de honorários: quando a carteira/caso é `acrescido_divida`, o valor pago pelo devedor é **rateado proporcionalmente** entre dívida-do-credor e honorários do escritório (SPEC §18), fechando exatamente em centavos; a parte da dívida é o que as alocações distribuem. Pós: `Pagamento` com composição (`valorDivida`, `valorEncargos`, `valorHonorarios`) + N `AlocacaoPagamento`; `Σ alocações === valorDivida + valorEncargos`; saldo derivado do caso cai; evento `pagamento_registrado`. Erros: caso não encontrado / de outro tenant; caso encerrado; obrigação de outro caso (invariável 12); soma das alocações diverge da parte da dívida rateada.

> **Sugestão FIFO** (pré-preencher a alocação por vencimento) é pré-preenchimento de UI e depende de saldo por obrigação — **fica para a Etapa 8** (telas). O UseCase da Etapa 3 recebe **alocação explícita**; a distribuição não é decidida no back nesta etapa.
...
- **RegistrarPagamento** (unit): rateio quando `acrescido_divida` (dívida/honorários separados); `Σ alocações === valorRecuperadoDivida` (senão `PagamentoInconsistenteException`); **pagamento não atravessa casos** (invariável 12 → `ObrigacaoDeOutroCasoException`); caso encerrado rejeita; evento `pagamento_registrado`.
