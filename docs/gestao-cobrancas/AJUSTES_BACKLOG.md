...
> **✅ Follow-up "acordo sobre acordo" na criação — FECHADO pelo ajuste 9** (`1a2a2df`). Ver item 9 abaixo.
...
## 8. Visualizar parcelas do acordo na aba Obrigações (dropdown + link) — `🔍 revisado (COMPLETO)`
> **Implementado:** partição no `MontarDetalheCasoUseCase::agruparPorAcordo()` (sem query nova) → `CasoDetalheOutput` ganha `gruposAcordo` + `obrigacoesAvulsas` (`obrigacoes` segue a lista COMPLETA, que alimenta o badge da aba); novo DTO `GrupoAcordoObrigacoesOutput`; `ObrigacaoOutput` +`acordoOrigemId`. Aba: acordo vigente vira **linha-resumo** ("Acordo {data} · Estado · N parcela(s) · substituiu M obrigação(ões) · R$ total" + **Abrir acordo**) com **accordion** das parcelas; lista solta abaixo. **Bônus:** `ObrigacaoRepository::doCaso` ganhou **fetch-join** de `acordoOrigem`/`acordoSubstituto` — matou um N+1 pré-existente (o `ObrigacaoOutput` lê o status dos dois por proxy); consumidor único, sem risco.
> **Regra de partição (a ORDEM importa):** (1) substituída por acordo vigente → **sai da aba** (vive no detalhe do acordo); (2) parcela viva de acordo vigente → **grupo**; (3) resto → lista solta (inclusive **parcela de acordo desfeito** = histórico editável, e original restaurada por rompimento). **Achado MÉDIO da revisão (corrigido):** com a ordem invertida, a parcela de A já substituída por B vigente (acordo-sobre-acordo — spec do item 7 §13) entrava no grupo de A **inflando o total contra o saldo derivado** e sendo contada 2× nos rótulos. Corrigido e **provado por mutação**; teste `testParcelaSubstituidaPorOutroAcordoNaoInflaOGrupo`. 6 functionals. `tests/Cobranca` **522/522**. **Sem migration.**
> **Aceito conscientemente (revisão):** o badge da aba segue = TOTAL de obrigações do caso (inclui as substituídas escondidas) — o resumo do grupo explica ("substituiu N"); os ramos de badge "Substituída"/"Parcela"/cadeado na lista solta ficaram inalcançáveis pela partição (mantidos como defesa, com comentário no template).
...
## 9. Bloquear "acordo sobre acordo" na criação — `🔍 revisado 2× (COMPLETO)` — commit `1a2a2df` — spec `docs/specs/cobranca-ajuste9-bloquear-acordo-sobre-acordo.md`
**Origem:** follow-up do item 7 §13, elevado após investigação. **Risco:** MÉDIO (dinheiro). **Sem migration.**

**Decisão de produto (humano, 2026-07-15):** renegociar a renegociação **NÃO é fluxo do negócio**. Para refazer um acordo: **rompa o atual** → as dívidas originais voltam ao saldo por derivação → acorde sobre elas.

**A investigação achou um vetor PIOR que o registrado no item 7** (que só analisou EDIÇÃO): romper/cancelar um acordo A cujas parcelas um acordo B vigente renegociou **DUPLICA a dívida no saldo** — as originais que A substituiu voltam ao exigível **E** as parcelas de B continuam nele. Derivável de `ObrigacaoRepository:109-110` + `CalculadoraSaldo:47`; nada compensava. Spec §2.1.

> **Implementado:** `ObrigacaoRepository::doCasoSubstituiveis` (só DÍVIDA ORIGINAL; **reusa `doCasoExigiveis`** em vez de duplicar critério — dentro do exigível, parcela ⇒ acordo vigente, logo o filtro é `acordoOrigem === null`) · guard no `CriarAcordoUseCase` (INV-I) + **exception própria** `ObrigacaoNaoEhDividaOriginalException` · guard no **romper E no cancelar** (mesmo vetor: os dois deixam o acordo não-vigente) via `parcelasRenegociadasPorAcordoVigente` + `AcordoComParcelasRenegociadasException` · UI: modal oferece só originais + texto do caminho certo + aviso no lugar da lista vazia (D3).
> **INV-J (o risco #1) preservado:** `doCasoExigiveis` **não mudou uma linha** — parcela de acordo vigente É exigível, e isso está certo. **INV-K:** modal de **pagamento** intocado (pagar parcela de acordo vigente é o fluxo mais usado; provado por HTTP real).
> **⚠️ ASSIMETRIA DELIBERADA — NÃO "CONSERTE" (spec §5.1.1):** o **render** (`MontadorModaisCaso::deMutacao`) filtra as substituíveis, mas o **POST** (`AcordoController::criar`) valida contra as **exigíveis**. Igualar as listas faz o ChoiceType barrar antes → guard e `catch` **inalcançáveis** → INV-L não-verificável. Foi o BLOQUEANTE da revisão.
> **🔴 BLOQUEANTE pego pela revisão (e corrigido):** o teste functional do `criar` **passava pelo motivo errado** — form inválido também dá 302 p/ a mesma URL sem criar acordo; provado por mutação (removendo o `catch`, seguia VERDE). Agravante achado ao corrigir: o `catch` estava **sem o `use`** da exception (nunca capturaria → 500) e **539 testes passavam verdes**. **Lição: seguro inalcançável não é seguro — é código morto que apodrece.** Corrigido: guard alcançável + teste que assere a **MENSAGEM**, provado por **mutação dupla** (sem catch → 500; sem guard → acordo criado).
> **Achado MÉDIO da revisão (corrigido):** reusar `ObrigacaoDeAcordoException` mentia pro gestor — ela diz "acordo **vigente**" e o guard também recusa parcela de acordo **rompido**. **Achado MÉDIO da re-revisão (corrigido):** docblock do teste sobreviveu à mudança de desenho e ensinava a reintroduzir o bloqueante.
> **Dívidas aceitas conscientemente:** `asub.tenant` não filtrado (fail-safe, nunca vazamento); botão de submit ativo com lista vazia (cosmético). Follow-ups em spec §8.
> `tests/Cobranca` **539/539** · global **1854/1854**. Smoke real no dev: acordo `4` criado no objeto 102 → 18 → 15 opções, nenhuma parcela oferecida, pagamento intacto.
