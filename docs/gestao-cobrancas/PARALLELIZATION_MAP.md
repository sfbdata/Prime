...
| **8** | Telas operacionais + UX — **decomposta em ondas 8A/8B/8C** | **Alta (8B)** | **ver ondas** | 2–7 | Etapa grande → ondas verificáveis. **8A ✅ (LEITURA/navegação): foi 1 escritor** — a fundação (Output DTOs → controllers → templates) é acoplada; fan-out não caberia. **8B ⬜ (mutações/forms): parallelizável 3–4** por grupos de ação independentes (Carteira/Objeto/Pessoa/Vínculo × Caso × Pagamento/Liquidação × Acordo × Ação/Revisão/Judicialização), **depois** de committar os contratos compartilhados (Form base, capacidades, rotas). **8C ⬜ (importação visual + file-manager de documentos): 1–2, acoplado** (pipeline de importação coeso; religar `pasta-arquivos.js` é 1 frente). |
