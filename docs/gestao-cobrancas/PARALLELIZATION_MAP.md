...
> **Estado (2026-07-10):** Etapas **0–7 + Etapa 8 Ondas 8A e 8B CONCLUÍDAS e committadas** na branch `gestao-cobrancas` (HEAD `69faab4`; `tests/Cobranca` 343/343; global 1624/1624). O fluxo de fan-out foi aplicado nas Etapas 2/3/4/5/6 e, na **Onda 8B**, de forma **híbrida**: o cluster do Caso (mutações caso-aninhadas — financeiro/judicializar/alterar-pessoa — que compartilham `caso/show.html.twig` + `_acoes_modais*`) rodou **single-writer** (orquestrador, test-driven), enquanto o cluster **Carteira** (8B-E cadastro, file-independente) rodou como **1 `feature-implementer` em worktree isolado → review read-only → cherry-pick individual → testes direcionados**. 2 revisões adversariais + tenant-safety SEM bloqueantes. A **Onda 8C** (importação visual + file-manager de documentos) é a próxima. Detalhe operacional em `AUTONOMOUS_EXECUTION_PROTOCOL.md` e `SESSION_HANDOFF.md`.
...
| **8** | Telas operacionais + UX — **decomposta em ondas 8A/8B/8C** | **Híbrida (8B)** | **ver ondas** | 2–7 | Etapa grande → ondas verificáveis. **8A ✅ (LEITURA/navegação): 1 escritor** — fundação acoplada (Output DTOs → controllers → templates). **8B ✅ (mutações/forms): HÍBRIDA** — o cluster do Caso (financeiro/judicializar/alterar-pessoa/acordo/ação/revisão) é **single-writer** (todos editam `caso/show.html.twig` + `_acoes_modais*` → NÃO file-independentes); o cluster **Carteira** (8B-E cadastro) rodou como **1 feature-implementer isolado → cherry-pick**. Lição: a paralelização "3–4" prevista não se sustentou no cluster do Caso — template compartilhado força single-writer; só o Cadastro era genuinamente disjunto. **8C ⬜ (importação visual + file-manager de documentos): 1–2, acoplado** (pipeline de importação coeso; religar `pasta-arquivos.js` é 1 frente). |
