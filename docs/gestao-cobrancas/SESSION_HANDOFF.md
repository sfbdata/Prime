...
> Sessão encerrada em: **2026-07-14 — RODADA DE AJUSTES no módulo (já em prod).** Itens 1, 2, 3, 4, 5 FECHADOS+commitados (detalhe abaixo/histórico). **Item 6 (pagamento auto-alocação FIFO + prévia ao vivo) — ✅ COMPLETO** (spec `docs/specs/cobranca-ajuste6-pagamento-fifo.md`; 3 fatias: `d8d61be` serviço FIFO + `4b726e9` FIFO-padrão nos UseCases + `66a3ece` UI/prévia). FIFO por padrão (dívida mais antiga), manual vira toggle avançado; prévia ao vivo dívida/honorários via endpoint GET (fonte única de centavos); sobrepagamento BLOQUEIA (teto = saldoExigivel derivado); relabel "Valor alocado"→"Valor nesta obrigação". **Sem migration.** Junto: **fix de bug pré-existente** (modal reutilizável com action nula → 405) + **XSS na prévia corrigido** (DOM/textContent). `tests/Cobranca` 449/449, global 1764/1764. Próximos: itens **7** (acordo inteligente — MÉDIO/ALTO, SPEC própria), **8** (parcelas na aba — BAIXO, depende do 7). Módulo segue no ar; nada desta rodada foi deployado ainda.
>
> **HEAD = `66a3ece`** (branch `gestao-cobrancas` local, NÃO pushada). Working tree limpo (+ commit de docs a seguir). Ordem dos commits do item 6: `d8d61be`(f1+spec) · `4b726e9`(f2) · `66a3ece`(f3). Pegadas dev inócuas do item 6: pagamento de teste no caso 295 (objeto 296) CRIADO e DEPOIS REMOVIDO durante debug do 405.
