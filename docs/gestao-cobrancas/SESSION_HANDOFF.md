...
> Sessão encerrada em: **2026-07-15 — ITEM 7 (acordo inteligente) ✅ COMPLETO (4 fatias).** Itens 1–7 FECHADOS+commitados. **Falta só o item 8** (parcelas agrupadas na aba Obrigações — BAIXO, depende do detalhe da Fatia 3, que já existe). Spec: `docs/specs/cobranca-ajuste7-acordo-inteligente.md` (decisões D1–D8; invariantes INV-A a INV-H). Detalhe no AJUSTES_BACKLOG §7.
>
> **HEAD = `bb10f90`** (branch `gestao-cobrancas` local, NÃO pushada). Commits do item 7: `361cb53`(F1 modelo+gerador) · `2c3ed78`(F2 gerador na UI) · `a2be47f`(F3 abrir acordo) · `bb10f90`(F4 editar acordo). **Migration `Version20260714130000`** (aditiva: `valor_total_negociado`+`valor_entrada`; snapshot NÃO-autoritativo p/ saldo) aplicada só em **dev+test** — prod é do humano no deploy. `tests/Cobranca` **516/516**. Nada da rodada foi pushado/deployado.
>
...
>
> **RETOMAR pelo item 8** (parcelas do acordo agrupadas na aba Obrigações, BAIXO): a aba hoje lista as parcelas soltas; vira linha-resumo ("Acordo {data} · N parcelas · R$ {total}") com accordion inline + link **"abrir acordo"** → `cobranca_acordo_show` (já pronto). Substituídas somem da lista principal (ficam no detalhe). **Cadência:** implementar→MOSTRAR smoke→humano aprova→suíte+/review+commit atômico.
