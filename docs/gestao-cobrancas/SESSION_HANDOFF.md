...
> Sessão encerrada em: **2026-07-14 (parte 2) — item 7 (acordo inteligente) EM ANDAMENTO.** Itens 1,2,3,4,5,6 FECHADOS+commitados (histórico abaixo). **Item 7 — Fatias 1 e 2 ✅ COMMITADAS; faltam Fatias 3 e 4.** Spec dedicada: `docs/specs/cobranca-ajuste7-acordo-inteligente.md` (decisões D1–D7 fechadas com o humano; invariantes INV-A a INV-H; fatiamento §11). Detalhe no AJUSTES_BACKLOG §7.
>
> **HEAD = `2c3ed78`** (branch `gestao-cobrancas` local, NÃO pushada). Working tree limpo (+ commit de docs a seguir). Commits do item 7: `361cb53`(F1 modelo+gerador) · `2c3ed78`(F2 gerador na UI). **Migration `Version20260714130000`** (aditiva: `valor_total_negociado`+`valor_entrada` em cobranca_acordo, snapshot NÃO-autoritativo p/ saldo) aplicada só em **dev+test** — prod é do humano no deploy. `tests/Cobranca` **475/475**.
>
> **RETOMAR pela Fatia 3 (abrir acordo — detalhe read-only):** `GET /cobrancas/acordos/{id}` (`cobranca_acordo_show`, gate MÓDULO, `findOneByIdDoTenant`→404) + `MontarDetalheAcordoUseCase` → `AcordoDetalheOutput` (readonly: entrada/total/desconto-derivado, parcelas com alocado/quitada, obrigações substituídas) + template `cobranca/acordo/show.html.twig` + link "abrir acordo" na aba Acordos do objeto; functional (render, gate módulo, cross-tenant 404, desconto derivado). **Depois Fatia 4 (editar acordo):** `EditarAcordoUseCase` (editor completo, diff, guardas INV-C parcela paga congelada / INV-D só Ativo / INV-E substituídas fixas / INV-H caso encerrado; evento novo `AcordoEditado` code-only) + `EditarAcordoInput`/`EditarAcordoType` + `POST cobranca_acordo_editar` + UI no detalhe. **Cadência:** implementar→MOSTRAR smoke→humano aprova→suíte+/review+commit atômico. Item **8** (parcelas agrupadas na aba) depende da Fatia 3.
