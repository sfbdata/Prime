...
| `13-escuro-honorarios-totais.png` | Aba **Honorários** com os dois cards no topo (`A receber` / `Já recebido`) e o rodapé `Total das obrigações`. O rodapé bate com a soma das seis linhas: 27,40 + 27,16 + 26,89 + 26,64 + 26,38 + 0,00 = **R$ 134,47** |
| `14-escuro-criar-acordo.png` | Modal **Criar acordo** em três passos, com dívidas como linhas clicáveis, `Selecionar todas`/`Limpar`, total da seleção em destaque e o status de fechamento fixo no rodapé, ao lado do botão que ele governa |
