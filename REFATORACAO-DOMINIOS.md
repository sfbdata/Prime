...
- **[RESOLVIDO - 2026-05-25] Filtro de cliente no PastaRepository (Acervo Geral)**: `findByFilters` e `findByFiltrosEMarcador` buscavam `p.nomeCliente` (campo denormalizado, quase sempre vazio) em vez da relação ManyToMany `pasta.clientes` — `ClientePF.nomeCompleto` e `ClientePJ.razaoSocial` nunca eram encontrados. Corrigido via `leftJoin(ClientePF/ClientePJ)` + `orX` + `GROUP BY p.id`. 2 novos testes em `PastaRepositoryIsolamentoTest`. Descoberto em produção. Detectado em 2026-05-21.
- **Filtro de NUP no PastaRepository (Expediente)**: NUP usa `LIKE` num campo que é select de valores exatos (deveria ser igualdade exata ou dropdown). Pendente — decidir junto com a frente de melhorias de filtro (busca livre). Detectado em 2026-05-21.
