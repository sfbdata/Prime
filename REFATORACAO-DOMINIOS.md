...
- **Doc de deploy desatualizada (nome do script)**: Project Instructions / docs referenciam `./scripts/deploy-prod.sh`, mas o script real de deploy em produção é `./scripts/deploy-prod-tls.sh`. Corrigir a referência para não induzir comando errado.
- **`deploy-prod-tls.sh` incompleto para 2 domínios**: o `echo` final anuncia só `https://grupojusprime.tech` e a checagem de certificado (linhas ~23-26) valida apenas o cert de `grupojusprime.tech`, ignorando `bluejus.com.br` (domínio principal, com usuários reais). O nginx serve ambos corretamente — problema é só no script de deploy. Detectado em 2026-05-20.
